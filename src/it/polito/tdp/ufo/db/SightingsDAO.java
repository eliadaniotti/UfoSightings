...
public class SightingsDAO {
	...
	public List<String> getAllStati(int anno) {
		String sql = "SELECT distinct state FROM sighting WHERE DATETIME LIKE '?%'" ;
	...
	public List<String> statiCollegati(String s1, int anno) {
		String sql = "SELECT distinct s2.state FROM sighting AS s1, sighting AS s2 WHERE s1.DATETIME < s2.DATETIME AND s1.state=? AND s2.state!=? AND s2.country='us' AND s1.datetime LIKE '?%' AND s2.DATETIME LIKE '?%'";
		List<String> list = new ArrayList<>() ;
